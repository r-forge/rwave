...
\item{nvoice}{number of scales in each octave (i.e., between two consecutive powers of 2).}
\item{w0}{central frequency of the wavelet.}
\item{twoD}{logical variable set to \code{T} to organize the output as a 2D array
(signal size \eqn{\times}{x} nb scales), otherwise, the output is a 3D
array (signal size \eqn{\times}{x} noctave \eqn{\times}{x} nvoice).}
\item{plot}{if set to \code{T}, display the modulus of the continuous wavelet
transform on the graphic device.}
}
\value{
list containing the continuous (complex) wavelet transform
and the phase derivative
\item{wt}{array of complex numbers for the values of the continuous wavelet transform.}
